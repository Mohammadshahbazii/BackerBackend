...
namespace Backer.Utilities
{
    ...
    {
        public static string GetMaxPersianDate(List<string> persianDates)
        {
            PersianCalendar persianCalendar = new PersianCalendar();

            // Parse Persian dates into DateTime objects
            var dateTimes = persianDates.Select(pd =>
            {
            ...
            DateTime maxDateTime = dateTimes.Max();

            // Convert the maximum DateTime back to a Persian date string
            string maxPersianDate = $"{persianCalendar.GetYear(maxDateTime)}/{persianCalendar.GetMonth(maxDateTime):00}/{persianCalendar.GetDayOfMonth(maxDateTime):00}";

            return maxPersianDate;
        }
        ...
        public static DateTime? ToShamsiDate(DateTime? date)
        {
           if (date == null)
            ...
            PersianCalendar pc = new PersianCalendar();
            DateTime dateTime = new DateTime(pc.GetYear(date.Value), pc.GetMonth(date.Value), pc.GetDayOfMonth(date.Value));
